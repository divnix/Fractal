...
in rec {
  ...
  in
    l.fromJSON (l.readFile (nixpkgs.runCommandLocal name {} ''
  ...
  compileHelmCharts = {
    ...
    versions = map (c:
      build c (attrByPath [c.source c.name c.version]
        (throw "No chart (${c.source}/${c.name}, ${c.version}) present in helm.lock.json; did you forget to run `fractal helm lock`?")
        attrs))
    config.helm.charts;
  in
    nixpkgs.symlinkJoin {
  ...
  compileJsonnet = {
    ...
    in
      nixpkgs.writeText "values.json" (l.toJSON full);
    ...
    '';
  in
    utils.recursiveTraverseResources (l.fromJSON (l.readFile result));
