          ...
          type:
            l.flatten (map (
              name: let
                path = p + "/${type}/${name}";
              in
                if !(l.pathExists (path + "/default.nix"))
                then []
                else makeModuleFromComponent {inherit type namespace name path;}
            ) (dirsFor (p + "/${type}")))
        ) (dirsFor p));
  in {
    ...
    kube = {
      ...
      in
        (ip xnlib.lib.recursiveModuleTraverse path) ++ components;
      ...
      helm = {
        ...
        sources = let
          ...
          attrs =
            if l.pathExists path
            then l.fromJSON (l.readFile path)
            else {};
        in
          xnlib.lib.recursiveMerge ((map (f: f.kube.helm.sources) flakes) ++ [attrs]);
      };
    };
  };
}
