...
in rec {
  ...
  defaultNamespaces = namespace: list:
    map (v:
      if ((l.attrByPath ["metadata" "namespace"] null v) != null)
      then v
      else v // {metadata = v.metadata // {inherit namespace;};})
    list;
  ...
  in
    (l.attrByPath path null resource) != null;
  ...
  defaultAnnotation = resource: annotation: value: let
    path = ["metadata" "annotations" annotation];
  in
    if ((l.attrByPath path null resource) != null)
    then resource
    else l.recursiveUpdate resource (l.setAttrByPath path value);
  ...
  fixupManifests = list:
    ...
    list [
      ...
      (m:
        if m.kind == "CustomResourceDefinition" || m.kind == "PersistentVolumeClaim"
        then
          l.recursiveUpdate m {
      ...
      (m:
        if m.kind == "ConfigMap"
        then l.filterAttrs (n: v: !(n == "data" && v == null)) m
        else m)
  ...
  in
    l.flatten (
      if l.isList object
      then map recursiveTraverseResources object
      else if l.isAttrs object
      then
        if isResource object
        then [object]
        else l.mapAttrsToList (_: v: recursiveTraverseResources v) object
      else throw "Key does not contain a Kubernetes resource!"
    );

  reduceStoreDir = name: path:
    builtins.path {
      inherit path name;
      recursive = true;
    };
