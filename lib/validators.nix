...
in rec {
  ...
  schemaNameFromGvk = gvk:
    l.toLower (l.concatStringsSep "-" (
  ...
  fetchAPISchema = version: let
    fetched =
      if !l.hasAttr version versionHashes
      then throw "No hash defined for version ${version}!"
      else
        l.fromJSON (l.readFile (nixpkgs.fetchurl {
  ...
  in
    xnlib.lib.recursiveMerge (l.mapAttrsToList (
        n: v: let
        ...
        in
          ...
          then {}
            ...
            l.listToAttrs (map (gvk:
              l.nameValuePair (schemaNameFromGvk gvk) {
                schema = filtered;
                path = "#/definitions/${n}";
              })
            gvks)
